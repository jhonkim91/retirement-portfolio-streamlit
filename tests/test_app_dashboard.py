...
class ThemeStylesheetTests(unittest.TestCase):
    ...
    def test_auth_stylesheet_hides_sidebar_container(self) -> None:
        ...
        self.assertIn('.st-key-dashboard-overview-hero-shell [data-testid="stElementContainer"]:has(.st-key-capture_input_panel):not(:has(.dashboard-overview-hero))', stylesheet)
        self.assertIn(".st-key-dashboard-overview-option2 .dashboard-overview-card-grid.dashboard-summary-strip {\n    display: grid !important;", stylesheet)
        self.assertIn("height: 220px !important;", stylesheet)
        self.assertIn(".st-key-dashboard-overview-option2 .dashboard-overview-card-grid.dashboard-summary-strip .dashboard-overview-card__sparkline", stylesheet)
        self.assertIn("grid-template-columns: repeat(2, minmax(0, 1fr)) !important;", stylesheet)
        self.assertIn("@media (max-width: 920px) {\n    .st-key-dashboard-overview-hero-shell .st-key-capture_input_panel [data-testid=\"stHorizontalBlock\"]", stylesheet)
        self.assertIn("width: 1.72rem !important;", stylesheet)
