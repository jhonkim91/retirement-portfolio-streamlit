...
- Altair는 기존 `5.0.1`보다 올리되 배포 런타임 호환성을 우선해 `altair>=5.3,<5.4`로 제한한다. 원래 검토값인 `5.4+`는 Streamlit Cloud Python 3.14 환경에서 `TypedDict(closed=True)` 임포트 오류가 재현되어 배포 복구 대상에서 제외한다. 참고: [Altair PyPI](https://pypi.org/project/altair/).
...
- `requirements.txt`의 `altair==5.0.1`을 `altair>=5.3,<5.4`로 변경한다.
